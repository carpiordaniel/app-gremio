...
import { ICategoria } from "../../../domain/parametros/categoria/interface/categoriaInterface";
export class CategoriaRepository  implements ICategoria {
  
  async getAllCategoria(): Promise<GremioModel[] | undefined> {
    try {
      const response = await fetch(`${API_URL}/web/obtenerCatalogoParametro/CATEGORIA`, {
        method: 'GET',
        headers: {
