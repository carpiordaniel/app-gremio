import { IGremio } from "../../../domain/parametros/gremio/interface/gremioInterface";
...
export class GremioRepository  implements IGremio {
  
  async getAllGremio(): Promise<GremioModel[] | undefined> {
    try {
      const response = await fetch(`${API_URL}/web/obtenerCatalogoParametro/GREMIO`, {
        method: 'GET',
        headers: {
      ...
      if (!response.ok) {  
        //if(response.status === 403 || response.status === 401 || response.ok === false){window.location.href = '/login'; }
